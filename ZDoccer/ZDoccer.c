...
static Gs   *_ProcessInlineConfluence( Gs       * const str, Para const * const para);
...
static int _Process(Gs const * const command, Gpath const * const path)
{
   ...
   if (gsIsEqualU2(command, L"all") ||
       gsIsEqualU2(command, L"confluence"))
   {
      _WriteConfluence(path, paraList);
   }
   if (gsIsEqualU2(command, L"all") ||
...
/******************************************************************************
func: _ProcessInlineConfluence

Process inline markup.
******************************************************************************/
static Gs *_ProcessInlineConfluence(Gs * const inStr, Para const * const para)
{
   Gs *chapter,
      *str;
...
static Gb _WriteConfluence(Gpath const * const path, ParaArray const * const paraList)
{
   ...
   gpathPushExtensionA(pathHtml, "con");
   ...
   forCount (index, paraArrayGetCount(paraList))
   {
      ...
      case paraTypeTABLE_ROW:
         if (isTableColHActive && !isTableColActive)
         {
            gfileSetA(file, gcTypeU1, " ||\n", NULL);
         }
         if (isTableColActive)
      ...
      case paraTypeTABLE_OF_CONTENTS:
         _WriteConfluenceTOC(file, paraList);
         break;
